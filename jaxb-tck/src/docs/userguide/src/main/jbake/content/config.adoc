...
4. Edit chosen file, for example `${TS_HOME}/lib/javasoft-multiJVM.jti`, and
  ...
  g.  Set `jck.env.jaxb.xsd_compiler.testCompile.xjcCmd` property to contain
      ...
      affect which CI is used in tests. 
...
[source,oac_no_warn]
----
TESTSUITE=/workspace/XMLB-TCK-4.1
WORKDIR=/workspace/XMLB-TCK-4.1/batch-multiJVM/work
...
jck.env.jaxb.schemagen.run.jxcCmd=/bin/sh
  /workspace/XMLB-TCK-4.1/bin/schemagen.sh
...
jck.env.jaxb.xsd_compiler.testCompile.xjcCmd=/bin/sh
  /workspace/XMLB-TCK-4.1/bin/xjc.sh
...
provide a script `$TS_HOME/bin/xjc.sh`,
which compiles schemas into class files.
The command is as follows:

`/bin/sh /workspace/XMLB-TCK-4.1/bin/xjc.sh`

For the Eclipse XML Binding implementation on
Windows, we provide a script `%TS_HOME%\bin\xjc.bat`,
which compiles schemas into class files.
The command is as follows:

`c:\workspace\XMLB-TCK-4.1\bin\xjc.bat`
...
Java Architecture for XML Binding 4.1
...
Eclipse XML Binding implementation on Linux we
provide a script `$TS_HOME/bin/schemagen.sh`, which
generates the schemas from the java files.
The command is as follows:

`/bin/sh /workspace/XMLB-TCK-4.1/bin/schemagen.sh`

For the Eclipse {TechnologyShortName} implementation on
Windows, we provide a script `%TS_HOME%\bin\schemagen.bat`,
which compiles the schemas into class files.
The command is as follows:

`c:\workspace\XMLB-TCK-4.1\bin\schemagen.bat`
