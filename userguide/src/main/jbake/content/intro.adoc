...
properties in the +{jtiSuffix}+ file you have chosen, and then use the JavaTest
harness to run the {TechnologyShortName} tests, as described in
...
details about the following steps.
  ...
  b.  Choose proper +{jtiSuffix}+ configuration file in the `${TS_HOME}/lib` directory.
  c.  Modify the required properties in the +{jtiSuffix}+ file you have chosen.
